# Prompt para correcao por etapas das auditorias tecnicas

> Copia de acompanhamento Codex.
>
> Legenda para execucao por partes:
> - `[ ]` Pendente / ainda nao validado no codigo atual
> - `[~]` Em andamento
> - `[x]` Corrigido e validado
> - `[!]` Requer decisao do dono do projeto antes de alterar
>
> Regra de uso: conforme cada lote for corrigido, este arquivo deve ser atualizado
> marcando o item correspondente como concluido e registrando a validacao feita.
...
- [x] C4 - Validar payload Manus com Zod em `server/storage.ts`. Corrigido com `ManusUrlResponse` e teste `server/storage.test.ts`.
- [x] A4 - Remover `refetchOnMount: "always"` das queries principais da Consultora, sem alteracao visual. Corrigido em `statusCounts`, `toWrite`, `pending` e `done`.
- [x] M1 - Logout deve limpar cookie com `maxAge: 0` explicito. Corrigido nos dois caminhos de logout (`server/routers.ts` e `server/routers/auth.ts`) e validado em `server/auth.logout.test.ts`.
- [x] C3 parte 1 - Exigir `MASTER_PASSWORD_HASH` sem fallback. Corrigido em `server/routers/security.ts`; `vitest.config.ts` define hash dummy apenas para testes.

Validacao do Sprint 0 parcial:

- `pnpm.cmd exec vitest run server/auth.logout.test.ts server/storage.test.ts`
