...
- **Performance Mobile (Consultora e Vendedor):** Os painéis do Vendedor e da Consultora devem ser altamente responsivos e rápidos, mesmo em celulares antigos. Prioridade: inputs grandes, teclado numérico, UX direta, sem travamentos.
...
- **Efeitos Visuais (Sutileza):** Animações e efeitos (lift effect em cards, hover em ícones, stagger em listas, fade entre páginas) devem ser fluidos e sutis. Confetes e fogos de artifício foram explicitamente removidos.

### 3. Regras de Negócio e Arquitetura

- **Paridade ADM e Consultora:** Toda funcionalidade implementada para a Consultora (gestão de trabalhos, status, reembolsos) deve ter um equivalente acessível no ADM.
...
- **Reembolsos no ADM:** O fluxo de reembolso ao cancelar uma consulta deve reutilizar o código e a aba já existentes no painel da Consultora — sem reinventar a roda.
- **Otimização de Banco e Queries:** O sistema teve problemas sérios com loops infinitos e excesso de conexões. Mantenha `staleTime` no React Query, nunca use `refetchInterval` agressivo, estabilize inputs de queries com `useMemo` e evite re-renders desnecessários.
