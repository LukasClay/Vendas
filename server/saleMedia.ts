...
function isStoredMediaRecord(value: unknown): value is SaleStoredMediaRecord {
...
export function getSaleClientPhotos(
  sale: Pick<
    SaleMediaFields,
    "photo1Url" | "photo2Url" | "photoExtras"
  >
): SalePublicPhoto[] {
...
export function findSalePhotoForDownload(
  ...
  return (
    getStoredPhotoExtras(sale).find(photo => photo.id === photoId) ?? null
  );
}

export function toPublicSale<T extends SaleMediaFields & Record<string, unknown>>(
  sale: T
) {
  const {
  ...
  return {
    ...
    attachments: getSaleAttachmentFiles(sale),
    clientPhotos: getSaleClientPhotos(sale),
  };
}
