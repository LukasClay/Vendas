  ...
  restore: adminProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await createAuditLog({
        userId: ctx.user.id,
        userName,
        action: "Restaurou Trabalho",
        details: JSON.stringify({ productId: input.id, name: product?.name }),
  ...
  create: adminProcedure
    .input(
      z.object({
        ...
        allowedCategories: z
          .array(z.enum(["individual", "promocao", "coletivo"]))
          .min(1, "Selecione pelo menos 1 tipo")
          .default(["individual", "promocao", "coletivo"]),
      })
    )
    .mutation(async ({ ctx, input }) => {
      ...
      await createAuditLog({
        userId: ctx.user.id,
        userName,
        action: "Criou Trabalho",
        details: JSON.stringify({
          name: input.name,
          categories: input.allowedCategories,
        }),
        ipAddress: ctx.ipAddress,
        userAgent: ctx.userAgent,
      });
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (product?.isSystem) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message:
            "Este trabalho é gerenciado pelo sistema e não pode ser editado.",
        });
      }
      const { id, ...data } = input;
      ...
      await createAuditLog({
        userId: ctx.user.id,
        userName,
        action: "Alterou Trabalho",
        details: JSON.stringify({ productId: id, changes: data }),
        ipAddress: ctx.ipAddress,
        userAgent: ctx.userAgent,
      });
      return { success: true };
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (product?.isSystem) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message:
            "Este trabalho é gerenciado pelo sistema e não pode ser excluído.",
        });
      }
