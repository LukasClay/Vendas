    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end">
          {slot.saleDate && (
            <span
              className="text-[10px] px-2 py-1 rounded-lg font-bold uppercase tracking-tighter"
              style={{
                background: "var(--secondary)",
...
export default function Consultas() {
  ...
  const [activeTab, setActiveTab] = useState<
    "pendentes" | "realizadas" | "canceladas" | "reembolsos" | "gerenciar"
  >("pendentes");
  ...
  const [confirmDeleteSlotId, setConfirmDeleteSlotId] = useState<number | null>(
    null
  );
  const [confirmRestoreId, setConfirmRestoreId] = useState<number | null>(null);
  const [confirmDeleteCancelledId, setConfirmDeleteCancelledId] = useState<
    number | null
  >(null);
  const [confirmApproveRefundId, setConfirmApproveRefundId] = useState<
    number | null
  >(null);
  const [confirmRejectRefundId, setConfirmRejectRefundId] = useState<
    number | null
  >(null);
  ...
  const approveRefundMutation =
    trpc.consultationSlots.approveRefund.useMutation({
      onSuccess: () => {
  ...
  const rejectRefundMutation = trpc.consultationSlots.rejectRefund.useMutation({
    onSuccess: () => {
      toast.success(
        "Reembolso rejeitado. Venda restaurada e consulta voltou para pendente."
      );
      invalidateAll();
    },
    onError: (err: any) => toast.error(err.message),
  ...
  const handleCreateSingleSlot = async () => {
    ...
    try {
      ...
      toast.success(
        `Horário ${selectedTime} criado para ${fmtDate(selectedDate)}!`
      );
      invalidateAll();
      setSelectedTime("");
    } catch (err: any) {
  ...
  const grouped = listToRender.reduce(
    (acc, s) => {
        ...
        <div className="flex gap-2 p-2 rounded-2xl bg-[var(--secondary)]/50 border border-[var(--border)] overflow-x-auto no-scrollbar snap-x snap-mandatory">
          {[
            ...
            },
            {
              id: "reembolsos",
              label: "Reembolsos",
              count: refunds.filter((r: any) => r.refundStatus === "pending")
                .length,
              icon: DollarSign,
            },
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                          {pending.map((r: any) => (
                            <motion.div
                              key={r.id}
                              layout
                              className="rounded-2xl p-5 border-2 border-amber-300 dark:border-amber-700 bg-[var(--card)] shadow-xl"
                            >
                              ...
                              <div className="flex gap-3">
                                ...
                                >
                                  ...
                                  approveRefundMutation.variables?.id ===
                                    r.id ? (
                                    <Loader2 className="w-4 h-4 animate-spin" />
                                  ) : (
                                    <ThumbsUp className="w-4 h-4" />
                                  )}
                                  {confirmApproveRefundId === r.id
                                    ? "Confirmar Aprovação"
                                    : "Aprovar"}
                                </motion.button>
                                <motion.button
                                ...
                                >
                                  ...
                                  rejectRefundMutation.variables?.id ===
                                    r.id ? (
                                    <Loader2 className="w-4 h-4 animate-spin" />
                                  ) : (
                                    <ThumbsDown className="w-4 h-4" />
                                  )}
                                  {confirmRejectRefundId === r.id
                                    ? "Confirmar Rejeição"
                                    : "Rejeitar"}
                                </motion.button>
                              </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                          {resolved.map((r: any) => {
                            ...
                            return (
                              <div
                                key={r.id}
                                className="rounded-2xl p-4 border border-[var(--border)] bg-[var(--card)] opacity-70"
                              >
                                <div className="flex items-center justify-between mb-2">
                                  <div className="flex items-center gap-2">
                                    <div
                                      className={`w-6 h-6 rounded-md flex items-center justify-center ${isApproved ? "bg-green-100 dark:bg-green-900/30" : "bg-red-100 dark:bg-red-900/30"}`}
                                    >
                                      {isApproved ? (
                                  ...
                                  <span
                                    className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${isApproved ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}
                                  >
            ...
            >
              <div className="text-center">
                ...
                <p
                  ...
                  style={{ color: "var(--muted-foreground)" }}
                >
                  Crie e gerencie horários de consulta
                </p>
              </div>
              ...
              <div className="max-w-md mx-auto space-y-3">
                ...
                <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 gap-1.5">
                  {TIME_OPTIONS.map(time => {
                    const alreadyExists = slots.some(
                      s =>
                        s.consultationDate === selectedDate &&
                        s.consultationTime === time
                    );
                    ...
                    return (
                      <button
                        ...
                        disabled={
                          alreadyExists || createSlotsMutation.isPending
                        }
                ...
                >
                  ...
                  {selectedTime
                    ? `Criar ${selectedTime} em ${fmtDate(selectedDate)}`
                    : "Selecione um horário"}
                </button>
              </div>

              <div
                className="max-w-2xl mx-auto rounded-2xl p-5 border border-[var(--border)]"
                style={{ background: "var(--card)" }}
              >
                <h2
                ...
                {loadingAll ? (
                  ...
                  <div className="space-y-2">
                    {[...slots]
                      .sort((a: any, b: any) => {
                        const dateComp = String(
                          a.consultationDate
                        ).localeCompare(String(b.consultationDate));
                        return dateComp !== 0
                          ? dateComp
                          : a.consultationTime.localeCompare(
                              b.consultationTime
                            );
                      })
                      .map((s: any) => (
                          ...
                          <div className="flex items-center gap-2">
                            {s.sold ? (
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <button
                                  onClick={() => {
                                    deleteSlotMutation.mutate({ id: s.id });
                                    setConfirmDeleteSlotId(null);
                                  }}
                                  disabled={
                                    deleteSlotMutation.isPending &&
                                    deleteSlotMutation.variables?.id === s.id
                                  }
                ...
                {!loadingAll && slots.length > 0 && (
                  <p
                    className="text-[10px] font-bold uppercase tracking-widest text-center mt-4"
                    style={{ color: "var(--muted-foreground)" }}
                  >
                    {slots.length} horário{slots.length !== 1 ? "s" : ""} no
                    total
                    {" • "}
                    {slots.filter((s: any) => s.sold).length} vendido
                    {slots.filter((s: any) => s.sold).length !== 1 ? "s" : ""}
                    {" • "}
                    {slots.filter((s: any) => !s.sold).length} disponíve
                    {slots.filter((s: any) => !s.sold).length !== 1
                      ? "is"
                      : "l"}
                  </p>
                )}
                      ...
                      {grouped[date].map(slot => (
                        <ConsultaCard
                          ...
                          cancelling={
                            ...
                            cancelMutation.variables?.id === slot.id
                          }
                          restoring={
                            restoreMutation.isPending &&
                            restoreMutation.variables?.id === slot.id
                          }
                          deleting={
                          ...
                          confirmingDelete={
                            confirmDeleteCancelledId === slot.id
                          }
                        />
                      ))}
                    </div>
