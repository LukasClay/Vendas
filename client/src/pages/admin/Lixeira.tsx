...
import { Trash2, RotateCcw, AlertTriangle, Clock, Search, Loader2, Package } from "lucide-react";
...
export default function AdminLixeira() {
  ...
  const filtered = deletedSales.filter((item: any) => {
    ...
    return (
      ...
      (item.sellerDisplayName || item.sellerName || "").toLowerCase().includes(term)
    );
  });
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <FadeIn>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold flex items-center gap-3" style={{ fontFamily: "'Playfair Display', serif", color: "var(--foreground)" }}>
                <Trash2 className="w-7 h-7" style={{ color: "var(--primary)" }} />
                Lixeira
              </h1>
              <p className="text-sm mt-1" style={{ color: "var(--muted-foreground)" }}>
                Vendas excluídas ficam aqui por 30 dias antes da exclusão permanente
              </p>
            </div>
            {expiredCount > 0 && (
              <motion.button
        ...
        {isLoading ? (
          ...
          <FadeIn>
            <div className="flex flex-col items-center justify-center py-20 gap-4 rounded-2xl border border-dashed border-[var(--border)] bg-[var(--card)]">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center bg-[var(--secondary)]">
                <Trash2 className="w-8 h-8" style={{ color: "var(--muted-foreground)", opacity: 0.3 }} />
              </div>
              <p className="text-lg font-bold" style={{ color: "var(--foreground)" }}>
                {searchTerm ? "Nenhum resultado encontrado" : "Lixeira vazia"}
              </p>
              <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>
                {searchTerm ? "Tente buscar por outro termo." : "Vendas excluídas aparecerão aqui."}
              </p>
            </div>
          </FadeIn>
        ) : (
          <StaggerList>
            <p className="text-xs font-bold uppercase tracking-wider" style={{ color: "var(--muted-foreground)" }}>
              {filtered.length} venda{filtered.length !== 1 ? "s" : ""} na lixeira
            </p>
            {filtered.map((item: any) => {
              const sale = item.sale ?? item;
              const days = daysRemaining(sale.deletedAt);
              const daysColor = getDaysColor(days);
              const sellerName = item.sellerDisplayName || item.sellerName || "—";
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                          <Clock className="w-3 h-3" style={{ color: daysColor }} />
                        ...
                        {sale.deletedAt && (
                          <p className="text-[10px]" style={{ color: "var(--muted-foreground)" }}>
                            Excluída em {new Date(sale.deletedAt).toLocaleDateString("pt-BR")} às {new Date(sale.deletedAt).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
                          </p>
                        )}
                      </div>
                        ...
                        {confirmDelete === sale.id ? (
                          <div className="flex items-center gap-2">
                            <motion.button
                              ...
                              animate={{ scale: 1 }}
                              whileTap={{ scale: 0.95 }}
                              onClick={() => permanentDeleteMutation.mutate({ id: sale.id })}
        ...
        <AnimatePresence>
          {confirmCleanup && (
            <div
              ...
              onClick={(e) => { if (e.target === e.currentTarget) setConfirmCleanup(false); }}
            >
              <motion.div
              ...
              >
                <div className="text-center mb-6">
                  ...
                  <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>
                    Isso vai excluir permanentemente <strong style={{ color: "#ef4444" }}>{expiredCount} venda{expiredCount !== 1 ? "s" : ""}</strong> que já passaram dos 30 dias.
                    <br /><br />
                    <strong style={{ color: "var(--foreground)" }}>Essa ação não pode ser desfeita.</strong>
                  </p>
                </div>
