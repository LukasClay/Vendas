...
function UrgencyChip({ daysRemaining, isOverdue, isDark }: { daysRemaining: number; isOverdue: boolean; isDark: boolean }) {
  if (isOverdue) return (
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <FadeIn>
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold" style={{ color: "var(--foreground)", fontFamily: "'Playfair Display', serif" }}>
                Alertas
              </h1>
          ...
          <StaggerList className="space-y-3">
            {alerts.map((item: any) => {
              const borderColor = item.isOverdue
                ? (isDark ? "oklch(0.30 0.10 25)" : "oklch(0.80 0.12 25)")
                : (isDark ? "oklch(0.30 0.08 55)" : "oklch(0.82 0.10 55)");
              const bgColor = item.isOverdue
                ? (isDark ? "oklch(0.16 0.02 25)" : "oklch(0.99 0.015 25)")
                : (isDark ? "oklch(0.16 0.02 55)" : "oklch(0.99 0.010 55)");
              return (
                <StaggerItem key={item.id}>
                  <div className="rounded-2xl p-4" style={{ background: bgColor, border: `1.5px solid ${borderColor}` }}>
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div className="flex-1 min-w-0 space-y-1.5">
                        <div className="flex items-center gap-2 flex-wrap">
                          <UrgencyChip daysRemaining={item.daysRemaining} isOverdue={item.isOverdue} isDark={isDark} />
                          <StatusBadge status={item.workStatus} isDark={isDark} />
                        </div>
                        ...
                        {item.clientPhone && (
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-xs" style={{ color: "var(--muted-foreground)" }}>{item.clientPhone}</span>
                          </div>
                        )}
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>Venda</p>
                        <p className="text-sm font-semibold" style={{ color: "var(--foreground)" }}>{formatDate(item.saleDate)}</p>
                        <p className="text-xs mt-1" style={{ color: "var(--muted-foreground)" }}>Prazo</p>
                        <p className="text-sm font-semibold" style={{
                          color: item.isOverdue
                            ? (isDark ? "oklch(0.75 0.18 25)" : "oklch(0.50 0.20 25)")
                            : (isDark ? "oklch(0.75 0.14 55)" : "oklch(0.45 0.18 55)")
                        }}>
